...
olc::ConsoleGameEngineOOP::ConsoleGameEngineOOP()
{
	m_nScreenWidth = 80;
	m_nScreenHeight = 30;
...
int olc::ConsoleGameEngineOOP::ConstructConsole(int width, int height, int fontw, int fonth)
{
...
void olc::ConsoleGameEngineOOP::Draw(int x, int y, wchar_t c, short col)
{
...
void olc::ConsoleGameEngineOOP::Fill(int x1, int y1, int x2, int y2, wchar_t c, short col)
{
	Clip(x1, y1);
	Clip(x2, y2);
	for (int x = x1; x < x2; x++)
...
void olc::ConsoleGameEngineOOP::DrawString(int x, int y, std::wstring c, short col)
{
...
void olc::ConsoleGameEngineOOP::DrawStringAlpha(int x, int y, std::wstring c, short col)
{
	for (size_t i = 0; i < c.size(); i++)
	{
		if (c[i] != L' ')
...
void olc::ConsoleGameEngineOOP::Clip(int &x, int &y)
{
	if (x < 0) x = 0;
...
void olc::ConsoleGameEngineOOP::DrawLine(int x1, int y1, int x2, int y2, wchar_t c, short col)
{
...
void olc::ConsoleGameEngineOOP::DrawCircle(int xc, int yc, int r, wchar_t c, short col)
{
	int x = 0;
	int y = r;
	int p = 3 - 2 * r;
...
void olc::ConsoleGameEngineOOP::FillCircle(int xc, int yc, int r, wchar_t c, short col)
{
...
void olc::ConsoleGameEngineOOP::DrawSprite(int x, int y, olc::Sprite *sprite)
{
	if (sprite == nullptr)
		return;
...
void olc::ConsoleGameEngineOOP::DrawPartialSprite(int x, int y, olc::Sprite *sprite, int ox, int oy, int w, int h)
{
	if (sprite == nullptr)
		return;
...
void olc::ConsoleGameEngineOOP::DrawWireFrameModel(const std::vector<std::pair<float, float>> &vecModelCoordinates, float x, float y, float r, float s, short col, wchar_t c)
{
...
void olc::ConsoleGameEngineOOP::Start()
{
	m_bAtomActive = true;

	// Star the thread
	std::thread t = std::thread(&olc::ConsoleGameEngineOOP::GameThread, this);

	// Wait for thread to be exited
	t.join();
}

int olc::ConsoleGameEngineOOP::ScreenWidth()
{
	return m_nScreenWidth;
}

int olc::ConsoleGameEngineOOP::ScreenHeight()
{
	return m_nScreenHeight;
}

void olc::ConsoleGameEngineOOP::GameThread()
{
...
bool olc::ConsoleGameEngineOOP::OnUserDestroy()
{
	return true;
}

int olc::ConsoleGameEngineOOP::Error(const wchar_t *msg)
{
	wchar_t buf[256];
...
BOOL olc::ConsoleGameEngineOOP::CloseHandler(DWORD evt)
{
...
unsigned int olc::ConsoleGameEngineOOP::LoadAudioSample(std::wstring sWavFile)
{
	if (!m_bEnableSound)
		return -1;
...
void olc::ConsoleGameEngineOOP::PlaySample(int id, bool bLoop)
{
...
bool olc::ConsoleGameEngineOOP::CreateAudio(unsigned int nSampleRate, unsigned int nChannels,
	unsigned int nBlocks, unsigned int nBlockSamples)
{
	...
	m_AudioThread = std::thread(&olc::ConsoleGameEngineOOP::AudioThread, this);
...
bool olc::ConsoleGameEngineOOP::DestroyAudio()
{
	m_bAudioThreadActive = false;
	return false;
}
...
void olc::ConsoleGameEngineOOP::waveOutProc(HWAVEOUT hWaveOut, UINT uMsg, DWORD dwParam1, DWORD dwParam2)
{
...
void CALLBACK olc::ConsoleGameEngineOOP::waveOutProcWrap(HWAVEOUT hWaveOut, UINT uMsg, DWORD dwInstance, DWORD dwParam1, DWORD dwParam2)
{
	((olc::ConsoleGameEngineOOP*)dwInstance)->waveOutProc(hWaveOut, uMsg, dwParam1, dwParam2);
}
...
void olc::ConsoleGameEngineOOP::AudioThread()
{
	m_fGlobalTime = 0.0f;
...
float olc::ConsoleGameEngineOOP::onUserSoundSample(int nChannel, float fGlobalTime, float fTimeStep)
{
	return 0.0f;
}

// Overriden by user if they want to manipulate the sound before it is played
float olc::ConsoleGameEngineOOP::onUserSoundFilter(int nChannel, float fGlobalTime, float fSample)
{
	return fSample;
}
...
float olc::ConsoleGameEngineOOP::GetMixerOutput(int nChannel, float fGlobalTime, float fTimeStep)
{
...
std::atomic<bool> olc::ConsoleGameEngineOOP::m_bAtomActive = false;
std::condition_variable olc::ConsoleGameEngineOOP::m_cvGameFinished;
std::mutex olc::ConsoleGameEngineOOP::m_muxGame;